...
import {
  ...
  Settings, LogOut, Sun, Moon, Boxes, PanelLeftClose, PanelLeftOpen
} from "lucide-react"
      ...
      <aside
        ...
        <nav className="flex-1 flex flex-col gap-0.5 p-1.5 overflow-y-auto">
          {visibleNav.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to}
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-2.5 py-2.5 rounded-md transition-all duration-150
        ...
        <div className="p-1.5 border-t space-y-0.5" style={{ borderColor: "var(--border)" }}>
          ...
          <button onClick={toggle}
            ...
            className="w-full flex items-center gap-3 px-2.5 py-2 rounded-md transition-all duration-150 hover:opacity-80"
